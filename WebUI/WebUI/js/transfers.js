...
THISPAGE = {
    init: function (a) {
        this.mod_PageConfig = Public.mod_PageConfig.init("transfers"),
        this.loadGrid(a),
        this.initDom(a),
        this.initCombo(),
        this.addEvent(),
    ...
    initDom: function (a) {
        ...
        if (b += g, this.btn_edit = c, this.btn_audit = e, this.btn_view = d, this.btn_reaudit = f, this.btn_tempSave = g, a.id > 0) {
            ...
            this.transfersListIds = parent.transfersListIds || [],
            this.idPostion = $.inArray(String(a.id), this.transfersListIds),
            this.idLength = this.transfersListIds.length,
            ...
            this.$_modifyTime.html(a.modifyTime),
            this.$_createTime.html(a.createTime),
            this.$_checkName.html(a.checkName)
        } else billRequiredCheck ? this.$_toolBottom.html("<span id=groupBtn>" + b + e + "</span>") : this.$_toolBottom.html('<span id="groupBtn">' + b + "</span>"),
    ...
    loadGrid: function (a) {
        function b(a, b, c) {
            return a ? (C(b.rowId), a) : c.invNumber ? c.invSpec ? c.invNumber + " " + c.invName + "_" + c.invSpec : c.invNumber + " " + c.invName : "&#160;"
        }
        function c(a, b) {
            var c = $(".goodsAuto")[0];
            return c
        ...
        function C(a) {
            ...
            if (b) {
                ...
                b.safeDays || ($("#grid").jqGrid("setCell", a, "prodDate", "&#160;"), $("#grid").jqGrid("setCell", a, "safeDays", "&#160;"), $("#grid").jqGrid("setCell", a, "validDate", "&#160;")),
                1 == b.isWarranty && $("#grid").jqGrid("showCol", "batch"),
                b.safeDays > 0 && ($("#grid").jqGrid("showCol", "prodDate"), $("#grid").jqGrid("showCol", "safeDays"), $("#grid").jqGrid("showCol", "validDate"));
        ...
        E = (new Date).format();
        if (a.id) {
            for (var F = 0; F < a.entries.length; F++) a.entries[F].id = F + 1;
            var G = 8 - a.entries.length;
            if (G > 0) for (var F = 0; G > F; F++) a.entries.push({})
        }
        D.newId = 9;
        var H = "grid",
        I = [{
        ...
        D.mod_PageConfig.gridReg(H, I),
        I = D.mod_PageConfig.conf.grids[H].colModel,
        $("#grid").jqGrid({
            data: a.entries,
            datatype: "clientSide",
            autowidth: !0,
            ...
            afterEditCell: function (a, b, c, d, e) {
                function f() {
                    ...
                    if (b) {
                        ...
                        b.inLocationId = c.inLocationId,
                        b.inLocationName = c.inLocationName,
                        b.outLocationId = c.outLocationId,
                        b.outLocationName = c.outLocationName,
                        $("#" + a).data("goodsInfo", b)
                    }
                }
                if (THISPAGE.curID = a, "goods" === b && (f(), $("#" + d + "_goods", "#grid").val(c), THISPAGE.goodsCombo.selectByText(c)), "skuName" === b) {
                    ...
                    "string" == typeof g.invSkus && (g.invSkus = $.parseJSON(g.invSkus)),
                    $("#" + d + "_skuName", "#grid").val(c),
                    THISPAGE.skuCombo.loadData(g.invSkus || [], 1, !1),
                    THISPAGE.skuCombo.selectByText(c);
                    var g = $("#" + a).data("goodsInfo");
                    if (!g) return;
                    SYSTEM.ISSERNUM && 1 == g.isSerNum && Business.serNumManage({
                        ...
                        beforeSet: function (a) {
                            a.outLocationName = a.locationName,
                            a.outLocationId = a.locationId
                        }
                    })
                }
                if ("qty" === b) {
                ...
                if ("outLocationName" === b) {
                    $("#" + d + "_outLocationName", "#grid").val(c),
                    THISPAGE.outStorageCombo.selectByText(c);
                    var g = $("#" + a).data("goodsInfo");
                    if (!g) return;
                    SYSTEM.ISSERNUM && 1 == g.isSerNum && Business.serNumManage({
                        row: $("#" + a),
                ...
                if ("mainUnit" === b) {
                    ...
                    if (!h.unitId || "0" === h.unitId) return $("#grid").jqGrid("restoreCell", d, e),
                    curCol = e + 1,
                    void $("#grid").jqGrid("nextCell", d, e + 1);
                    ...
                    if (!f) {
                        ...
                        g ? g : ($.dialog({
                            ...
                            button: [{
                                ...
                                focus: !0,
                                callback: function () {
                                    return this.content.callback && this.content.callback(),
                                    !1
                                }
                            },
                            {
            ...
            afterSaveCell: function (a, b, c, d, e) {
                switch (b) {
                    ...
                    case "batch":
                        ...
                        if (g.safeDays) {
                            var h = {};
                            if ($.trim(f.prodDate) || (h.prodDate = E), $.trim(f.safeDays) || (h.safeDays = g.safeDays), !$.trim(f.validDate)) {
                    ...
                    case "prodDate":
                        ...
                        g = $("#" + a).data("goodsInfo") || {},
                        h = {};
                        $.trim(f.safeDays) || (h.safeDays = g.safeDays),
                        $.trim(c) || (h.prodDate = E);
                        var i = c || h.prodDate,
                        j = i.split("-");
                        if (i = new Date(j[0], j[1] - 1, j[2]), "Invalid Date" === i.toString()) return defaultPage.Public.tips({
                        ...
                        10);
                        i && (i.addDays(Number(f.safeDays || h.safeDays)), h.validDate = i.format()),
                        $("#grid").jqGrid("setRowData", a, h)
                }
            },
            loadonce: !0,
            resizeStop: function (a, b) {
                D.mod_PageConfig.setGridWidthByIndex(a, b, "grid")
            },
            footerrow: !0,
            userData: {
                goods: "合计：",
        ...
        this.outStorageCombo = $(".storageAuto").combo({
            ...
            callback: {
                onChange: function (a) {
                    ...
                    c || (c = {}),
                    a && (c.id = a.id, c.name = a.name),
                    b.data("storageInfo", c)
                }
            }
        }).getCombo(),
    ...
    addEvent: function () {
        ...
        $("#batch-storageA").powerFloat({
            ...
            reverseSharp: !0,
            target: function () {
                return null !== curRow && null !== curCol && ($("#grid").jqGrid("saveCell", curRow, curCol), curRow = null, curCol = null),
                $(".wrapper").data("batch", "storageA"),
                $("#storageBox")
            }
        }),
        $("#batch-storageB").powerFloat({
            ...
            target: function () {
                return null !== curRow && null !== curCol && ($("#grid").jqGrid("saveCell", curRow, curCol), curRow = null, curCol = null),
                $(".wrapper").data("batch", "storageB"),
                $("#storageBox")
            }
        }),
        $(".wrapper").on("click", "#storageBox li",
        function (a) {
            var b = $(this).data("id"),
            c = $(this).data("name"),
        ...
        Business.billsEvent(a, "transfers"),
        ...
        function (a) {
            a.preventDefault(),
            Business.verifyRight("TF_ADD") && parent.tab.overrideSelectedTabItem({
        ...
        function (b) {
            ...
            function (a) {
                THISPAGE.reloadData(a.data),
                $("#next").removeClass("ui-btn-next-dis"),
                loading && loading.close()
            })), void 0)
        }),
        $("#bottomField").on("click", "#next",
        function (b) {
            return b.preventDefault(),
    ...
    _getEntriesData: function () {
        for (var a = [], b = $("#grid").jqGrid("getDataIDs"), c = 0, d = b.length; d > c; c++) {
            ...
            if ("" !== g.goods) {
                ...
                $("#grid").jqGrid("editCellByColName", f, "skuName"),
                !1;
                if (!i || !i.id) return parent.Public.tips({
                    type: 2,
                    content: "请选择调出仓库！"
                }),
                $("#grid").jqGrid("editCellByColName", f, "outLocationName"),
                !1;
                if (!j || !j.id) return parent.Public.tips({
                    type: 2,
                    content: "请选择调入仓库！"
                }),
                $("#grid").jqGrid("editCellByColName", f, "inLocationName"),
                !1;
                if (SYSTEM.ISSERNUM) {
                    var m = h.serNumList;
                    if (m || m && m.length != h.qty);
                        ...
                        $("#grid").jqGrid("editCellByColName", f, "qty"),
                        !1
                    }
                }
                e = {
